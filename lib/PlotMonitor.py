...
TIMING_TEST = False
MINIMUM_TIMESTAMP = datetime.strptime(
    "2000-01-01T00:00:00", "%Y-%m-%dT%H:%M:%S"
)  # earlier dates considered corrupt
...
class RetrieveData:
    ...
    def __init__(self, plotData):
        ...
        self.startDate = datetime.strptime(
            self.plotData["plot_from"], "%Y-%m-%d %H:%M:%S"
        )

        if self.plotData["plot_to"] != "now":
            self.endDate = datetime.strptime(
                self.plotData["plot_to"], "%Y-%m-%d %H:%M:%S"
            )
        else:
        ...
        if "data" not in self.plotData or (
            "data" in self.plotData and len(self.plotData["data"]) == 0
        ):
    ...
    def downloadDailyData(self, currDate, abm, lru, monitor):
        ...
        url = UrlTools.BuildMonPointUrl(
            currDate.strftime("%Y-%m-%d"), abm, lru, monitor
        )
        ...
        #    print('Skipping malformed file %s...' % url)
            ...
            self.logger.error(
                "ERROR: Could not fetch URL %s.  Trying for a second time..." % url
            )
            sleep(1)
            try:
                raw = pd.read_csv(url, sep=" ", header=None, engine="c", prefix="col")
    ...
    def retrieve(self):
        ...
        for idx, plotter in enumerate(self.plotData["data"]):
            ...
            self.logger.info(
                "\nProcessing %s / %s / %s / col%d"
                % (plotter["abm"], plotter["lru"], plotter["monitor"], dataCol)
            )

            # If present, used cached data files for current data set
            dataFileName = "%s_%s_%s_%s_%ddays.hdf" % (
                plotter["abm"],
                plotter["lru"],
                plotter["monitor"],
            ...
            if (
                not self.noCache
                and not signalNoCache
                and path.isfile(path.join(self.tempDir, dataFileName))
            ):
                self.logger.info(
                    "Found cached data file %s" % path.join(self.tempDir, dataFileName)
                )
                ...
                for currDate in map(
                    lambda x: self.startDate + timedelta(days=x),
                    range(self.numMonFiles),
                ):
                    usingAlternativeMonitor = False

                    raw, ex = self.downloadDailyData(
                        currDate, plotter["abm"], plotter["lru"], plotter["monitor"]
                    )
                    ...
                    if ex is not None:
                        ...
                        # Empty file, attempt alternative lru or monitor point name, if available
                            ...
                            if (
                                "monitor_alternative" in plotter
                                and plotter["monitor_alternative"] != ""
                            ):
                                self.logger.info(
                                    "Attempting alternative monitor point %s"
                                    % plotter["monitor_alternative"]
                                )
                                ...
                                # Experimental: construct multi-column Pandas dataframe from alternative monitor point list
                                    ...
                                    for i, monitorAlternative in enumerate(
                                        plotter["monitor_alternative"]
                                    ):
                                        rawList[i], ex = self.downloadDailyData(
                                            currDate,
                                            plotter["abm"],
                                            plotter["lru"],
                                            monitorAlternative,
                                    ...
                                    for i, rawAlternative in enumerate(rawList):
                                        ...
                                        if rawAlternative is not None:
                                            raw["col%d" % (i + 1)] = (
                                                rawAlternative.iloc[:, -1:]
                                            )
                                        else:
                                            raw["col%d" % (i + 1)] = np.nan
                                    usingAlternativeMonitor = True
                            elif (
                                "lru_alternative" in plotter
                                and plotter["lru_alternative"]
                            ):
                                self.logger.info(
                                    "Attempting alternative lru name %s"
                                    % plotter["lru_alternative"]
                                )
                                raw, ex = self.downloadDailyData(
                                    currDate,
                                    plotter["abm"],
                                ...
                                if ex is not None:
                                    self.logger.info(
                                        "No data found on alternative lru name %s. Skipping..."
                                        % plotter["lru_alternative"]
                                    )
                                    continue
                    ...
                    dataCol = (
                        int(plotter["data_column"])
                        if not usingAlternativeMonitor
                ...
                if not "col%d" % dataCol in data.columns:
                    self.logger.info(
                        "No column %d found in data. Skipping signal." % dataCol
                    )
                    continue
                ...
                initLen = len(data)
                self.logger.info(
                    "Processing and filtering data. %d monitor points. Please wait..."
                    % initLen
                )
                # sys.stdout.flush()

                # Convert timestamp column to datetime format
                data.col0 = pd.to_datetime(
                    data.col0, format="%Y-%m-%dT%H:%M:%S.%f", errors="coerce"
                )
